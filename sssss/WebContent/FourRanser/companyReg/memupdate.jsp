...
<%
	...
	mem.setId(request.getParameter("name"));
	dao.meminfo(mem);
%>
...
<table border=1 style="render">
		<tr>
			<td>id</td>
			<td>
			<input type="<%=mem.getId()%>" name="id" size="10" maxlength="8" />
			<input type="button" name="sch" class="sch" value="ID중복확인" />
			</td>
		</tr>
		<tr>
			<td>pw</td>
			<td><input type="password" name="pw" /><%=mem.getPw()%></td>
		</tr>
		<tr>
			<td>성별</td>
			<td><input type="radio" name="gender" checked="<%=mem.getGender()%>" value="male"/>남
